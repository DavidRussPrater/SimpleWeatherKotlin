package com.example.android.simpleweather.ui.main.tomorrow
...
class TomorrowsForecastFragment : Fragment() {
    ...
    fun render(weather: CurrentAndToday){
        ...
        //mBinding.detailTomorrowPrimary.tomorrowsDate.text =  unixDate(weatherList.daily.get(1)).toString()
...
//        mBinding.detailTomorrowSun.tomorrowsSunset.text = unixTime(weatherList.daily.get(1).sunset).toString()
        ...
        with(mBinding.tomorrowsHourlyForecast.tomorrowsHourlyRecyclerView){
            ...
            adapter = com.example.android.simpleweather.adapters.TomorrowHourlyAdapter(weather.hourly)
        }
    }

    fun String.capitalizeWords(): String = split(" ").map { it.capitalize(Locale.ROOT) }
        .joinToString(" ")
